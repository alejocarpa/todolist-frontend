...
.then( json => {
    ...
    selectEstados.innerHTML = `<select id="estado" class="form-select form-select-sm" onchange="filtroTareas( estado.value, usuario.value )"><option value="" selected>Todos</option>${ json.results.map( estado => `<option value="${ estado.id_estado }">${ estado.nombre_estado}</option>`) }</select>`;
})
...
.then( json => {
    ...
    selectUsuarios.innerHTML = `<select id="usuario" class="form-select form-select-sm" onchange="filtroTareas( estado.value, usuario.value )"><option value="" selected>Todos</option>${ json.results.map( usuario => `<option value="${ usuario.nombre_usuario_tarea }">${ usuario.nombre_usuario_tarea}</option>`) }</select>`;
})
.catch(err => console.log(err));
...
const filtroTareas = ( id_estado, usuario ) => {
    ...
    .then( json => {
        ...
        while (cuerpoTareas.firstChild) {
            cuerpoTareas.removeChild(cuerpoTareas.firstChild);
        }
